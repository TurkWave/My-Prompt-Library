...
State the resolved report language explicitly in Chat Summary Section A ("Report language: English, the default, since no target language was specified"). Never leave the field blank or silently guess a language. SPDX identifiers, license names, and file paths always stay in their original form.

## HARD LIMITS (Non-negotiable)
- Never modify, delete, move, rename any project file, or propose new code/deps/files. Sole output: the license report, in the file layout defined by the **Output File Splitting** rule below (single `LicenseChecked.md` **written at the project root**, or a split multi-file set under `docs/licenses/`, and never anything else). No writes to the project outside the report files. *(This is the complete and only statement of the write-scope restriction in this document, and it is not repeated elsewhere.)*
- Never assign a license by assumption. With no evidence, write **UNCERTAIN**.
- **No risk interpretation, ever, on any basis.** Not from distribution context, not from package popularity, not from "everyone uses this." You surface evidence, and the reader makes the legal judgment. This applies to every section of this document, including Distribution Context Note and Critical Findings. There are no exceptions, and this is the only place this rule is stated. The only permitted summary line is a **raw tally** of VIOLATION/CONFLICT/UNCERTAIN counts already assigned by evidence rules (e.g. "0 VIOLATION, 2 CONFLICT, 5 UNCERTAIN"). Words like "clean," "needs attention," "safe," "risky," "high risk" and "low risk" are forbidden anywhere in the report or chat summary. A count is a fact, an adjective is a judgment. The closed status set is defined once in FINDING STATUS VOCABULARY.
...
- Their licenses are recorded verbatim with evidence, and their original notices are reported as present or absent. No judgment is made on whether vendoring them is permissible, because that is the reader's call.
...
If a project has multiple manifest types, such as `package.json` plus `Cargo.toml` plus `go.mod`, each ecosystem is scanned as a **separate dependency tree**, and in the report Sections 4 and 5 split into per-ecosystem subtables (`4.1 Node.js`, `4.2 Rust`, `4.3 Go`). Trees are never merged, because version conflicts and license differences are meaningless across ecosystems.

**Dedup key includes ecosystem tag.** The dedup key for transitive dependencies (see Item 2 above) is `ecosystem:package@version`, never `package@version` alone. Two different ecosystems can coincidentally share a package name, and trees are never merged, as the previous sentence says, so the dedup key must not merge them either.

**Worked example: split-output file layout for a monorepo with npm and Cargo, both over threshold:**
```
docs/licenses/
...
Single-file output risks hitting the model's max output token limit on large monorepos with many ecosystems and 1000 or more transitive dependencies. A truncated report is a silent data-loss failure, and it is worse than a split report. This is decided in **Phase 1**, before any writing starts:

- **Default:** single `LicenseChecked.md`.
- **Split trigger:** if the project is multi-ecosystem (Monorepo section applies) **and** estimated total report rows (project files + direct + transitive deps + assets, from the Phase 0 inventory) exceed **~800 rows**, OR any single ecosystem's subtree alone exceeds **~400 rows**, split the output **per ecosystem** using the file layout shown in the Monorepo worked example above.
- This is a **file-count decision, not a batching decision**. It is independent of the Batching Plan (PHASE 1), which governs processing and writing order within Phase 2, not how many output files exist. These two thresholds are deliberately set to different numbers (this rule uses 800 and 400 rows, the Batching Plan below uses 150 files and 300 dependencies) specifically so they never numerically coincide and read as the same rule. State the chosen layout explicitly in the Phase 1 plan presentation.
...
- **Class 1, Standard Permissive. A CLOSED LIST, with no `etc.`:** exactly `MIT`, `MIT-0`, `Apache-2.0`, `BSD-2-Clause`, `BSD-3-Clause`, `ISC`, `0BSD`, `Zlib`, `BSL-1.0`, `Unlicense`, `CC0-1.0`, `PostgreSQL`, `NCSA`. The registry declaration **alone suffices**, giving Status: **Verified (registry)**. The repo `LICENSE` is not additionally pulled.
  - **Staleness guard:** registry metadata can lag behind an actual license change upstream (author relicenses, registry entry not updated). If the registry response exposes a last-published or last-updated date for the queried version and it is more than 18 months old, do not silently trust it. Append `(stale-unverified)` to the status (`Verified (registry, stale-unverified)`) and list it in Section 7 (Uncertain Items) as a soft flag. This is a **flag for the reader, not an auto-escalation**. The agent does not automatically re-run Class 2 or 3 dual-source verification on a stale Class 1 package, because that would silently change its risk classification, and only the reader's own judgment should trigger that. If the registry response exposes no such date, proceed as normal `Verified (registry)`. Do not fabricate a date to trigger the flag.
...
- If the **repo source is unreachable** (private repo, deleted, and so on), which **does not apply to Class 1**, since Class 1 never attempts a repo pull in the first place (see above):
  **Class 2 and 3**: this does NOT count as "Verified". The second verification failed, so mark **UNCERTAIN (single source, second verification unavailable)** and place it in Section 7 (Uncertain Items) rather than Critical Findings, with the reason. *Rationale: treating a single source as "verified" in the class needing the most verification produces false confidence, because it means the riskiest package class went unevidenced.*
...
4. **Verifiable upstream source.** A source URL recorded in-project that resolves to a page stating the asset's license terms, which is external evidence per the Evidence Rule.
...
Build an inventory only: directory structure, languages/frameworks, dependency manifests, existing LICENSE file, project size. Out-of-Scope directories excluded. No code logic analysis and no license or copyright findings are produced yet. That begins in Phase 2. *(This phase's output feeds Phase 1's plan. It is not the "scanning" that Phase 1 refers to when it says no scanning yet, see Phase 1.)*

**Network restriction:** No **external HTTP/registry queries** in this phase. CLI tools (`syft`, `pip-licenses`, `license-checker`) **in local-parse mode**, meaning they read and structure only the on-disk manifest or lockfile with no network, may be used here. Reading tool output as JSON, instead of raw-reading massive files such as a 150-dependency `pnpm-lock.yaml` or `Cargo.lock` into LLM context, avoids token waste and line-skipping errors. What is forbidden is the network request, not CLI usage.

**If a local-parse-mode CLI tool is unavailable in this phase:** fall back to a direct manifest or lockfile read, using the same static-read approach defined for Phase 2 (see the PHASE 2 fallback), scoped here to inventory only, meaning file and dependency counting, not license extraction. Note the fallback in the Phase 1 plan's "Tools used" field, same as a Phase 2 fallback would be noted.

**Pre-Risk Scan (conditional):** runs before plan presentation when the dependency count is **more than 150 transitive dependencies**. This is a *dependency-count* trigger, distinct from the *file-count and content-volume* trigger used by the Batching Plan below. The two use different units, dependencies against files, and the shared value 150 is coincidental rather than a shared rule. It is skipped for small projects.
...
Based on Phase 0 results, **present** the following. No findings are produced and no report file is written yet, per the Phase 0 note on "scanning" terminology:
- **Report output language.** The resolved language and how it was resolved, in one line, for example "Report language: English, the default, since no target language was specified" or "Turkish, explicitly requested by the user". It is stated here, before approval, so a wrong language is caught before any writing starts. See the LANGUAGE section.
- **Output layout decision.** A single `LicenseChecked.md` at the project root, or a split under `docs/licenses/` (per Output File Splitting).
...
- **Batching plan** (stated as "single batch" for small projects):
  - The split threshold is based on estimated **context volume**, not on file or package **count**. Even under the count threshold, minified or large content still triggers a split. Rough threshold: more than 150 files, more than 300 dependencies, or a total content volume too large for single-pass processing. *(This threshold governs processing and writing order within Phase 2 only. It is a different rule from the Output File Splitting section's 800 and 400 row thresholds, which govern how many output files exist. The two are set to different numbers specifically so they never read as the same rule.)*
...
**Stop after this phase and wait for approval. Phase 2 does not begin without approval.** *(This is the sole approval gate in the entire workflow. See the Critical-finding rule in Phase 2, which explicitly does not reopen it.)*
...
**Tool usage:** dependency scanning runs via CLI tools, and hundreds of packages are not read one by one as text. By ecosystem:
...
- **If none installed/accessible (fallback):** tool absence is detected on first package attempt (command not found/error) → from that point, switch to **static file-read mode** for that ecosystem: parse manifest/lockfile directly (as JSON/TOML/YAML), extract the `license` field. This switch is reported to the user with a one-time brief note ("Tool X unavailable, switched to static file read"), flow does not stop.
  - **If the extracted `license` field is itself empty or missing**, which is common in real-world manifests, this is not a dead end. It is a normal classification outcome, treated identically to "registry license field empty" under Risk-Based Cross-Verification, which makes it **Class 3** with full verification mandatory and the repo `LICENSE` pulled. Do not write UNCERTAIN at this step merely because the manifest field was blank. UNCERTAIN is only for when Class 3's own verification, the repo pull, also fails.
...
5. Write to the target report file **incrementally**, meaning `LicenseChecked.md`, or the relevant per-ecosystem file plus `_index.md` if Output File Splitting triggered. Accumulate as you go rather than in bulk at the end, so an interruption doesn't lose work. See the Output Truncation and Resume Protocol above for mid-write cutoffs specifically. Save after each batch in the batching plan completes. *(This is the only phase where the write operation merely planned in Phase 1 actually happens.)*
...
2. Confirm the report is complete: a single `LicenseChecked.md`, or, if Output File Splitting triggered, `docs/licenses/_index.md` plus all per-ecosystem files, each verified present and not truncated.
...
- Verification-depth distribution (Class 1, registry single-source: 340; Class 2 and 3, dual-source: 15; Class 2 and 3, UNCERTAIN single-source: 3).
- **Critical findings list**, if any. A one-line summary each, plus "detail in [report file]", naming the specific per-ecosystem file if split and `LicenseChecked.md` otherwise. Cross-verification conflicts are marked separately.
...
- **`_index.md`** carries Sections 1 (including 1.1 to 1.3), 2 (aggregated), 3 (including 3.1), 6 and 9 (aggregated), plus the ecosystem link table. It also carries the project-wide part of Section 10: the vendored and asset rows, plus the redistribution totals summed across every ecosystem.
- **each per-ecosystem file** carries Sections 1, 2, 4, 5, 7, 8 and 9 scoped to its own ecosystem, plus its own Section 10 dependency rows. It does **not** carry Sections 3 or 6, which are project-wide and exist only in `_index.md`.
...
- Pre-Risk Scan performed (the 150-dependency threshold): Yes / No / Not applicable, dependency count below threshold
...
`NOTICE` and `THIRD-PARTY-NOTICES` files at the project root and any shipped by dependencies (recorded presence and path only, since contents are not interpreted).
...
(Same format, version-locked, deduplicated by `ecosystem:package@version`. If the same key repeats across multiple parents *within this ecosystem*, it is listed once with a "Used by" column. A same-named package in a different ecosystem is always a separate entry, never merged.)
...
## 9. Conclusion
Counts only: total items scanned, SPDX distribution, VIOLATION count, CONFLICT count, UNCERTAIN count (7.1 and 7.2 separately), vendored-subtree count, distinct copyright holders, redistribution split (`shipped`, `build-only`, `UNCERTAIN`), distinct license texts across the shipped set, the count of shipped items not named in any in-project notice file, and the deviation between the Phase 1 estimate and the Phase 2 actual, if any. No summarizing adjective, no recommendation, no next steps.
...
- All other rules (write-scope, the risk-interpretation ban, the approval gate) are stated exactly once each, at their point of first relevance above. See HARD LIMITS, PHASE 1 and PHASE 2 respectively. Nothing in this section overrides them; this section adds no new rules beyond the two above.
