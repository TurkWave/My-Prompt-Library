# TASK: Project Licensing, Analysis, Selection and Implementation
...
> drafts in the `Licenses/` folder, filled in and never rewritten.
> It then assembles the **third-party notices** that the code the
> project redistributes requires it to carry (Section 0.3). That is a
> second deliverable, independent of which license was chosen.
> It does not produce the audit report, and it does not author
> license text.

## LANGUAGE
**Two separate channels. Never conflate them.** *(This is the complete and only statement of the language rule in this document, and it is not repeated in Section 5.)*

- **File output is English by default.** That covers everything this task WRITES into the project: the LICENSE file, SPDX headers, copyright lines, the README license section, manifest field values, and any file content produced. It does NOT follow the audit report's language, nor the language the user writes in. It changes only when the user **explicitly asks** for the file content in another language. Official license texts are always reproduced in their original official form and are never translated, regardless of this setting. The same split applies inside the third-party notices deliverable (Section 0.3): the register's own prose, meaning its headings, column labels and explanatory lines, is written in the file-output language, while every reproduced copyright line and every reproduced license text stays byte-for-byte as its author wrote it, in its own language.
- **Conversation follows the user.** That covers plan presentation, recommendations, questions, approval requests and the Section 6 summary. Use the language the user writes in, and switch when the user explicitly asks for another language or simply starts writing in one. A Turkish report or a Turkish conversation NEVER makes the file output Turkish, and the file-output language never dictates the conversation language.

State the resolved file-output language in one line when presenting the PHASE 1 plan (for example "File output language: English, the default").

## 0. PRECONDITION (MANDATORY, THE TASK DOES NOT START WITHOUT THIS)
This task REQUIRES a "License Audit Report" to have been provided, as a
mandatory precondition. That is a document produced by scanning the
project's files, dependencies and assets, laying out the current
license status.
...
- **Split set.** `docs/licenses/_index.md` plus one file per
  ...
  Section 10 (the redistribution and attribution inventory: vendored and
...
- If this report was not provided: do not perform any analysis,
  ...
  license selection would be evidence-free and assumption-based, which
...
conflict, regardless of whether it's license text, an SPDX
identifier, or a compliance rule):** the license's official text /
...
say so explicitly in the plan, in the form "compatibility of X with Y
could not be externally verified", and do not present the unverified
...
- **File name = SPDX identifier.** `MIT.txt` gives `MIT`,
  `Apache-2.0.txt` gives `Apache-2.0`, `CC-BY-SA-4.0.txt` gives
...
one word, not one line break, not one blank line, and no re-wrapping.
...
State the result explicitly in the step report, for example "LICENSE
...
- **GNU family, where the version suffix is mandatory.** SPDX has
  deprecated the bare identifiers. `GPL-2.0.txt` becomes
  `GPL-2.0-only` or `GPL-2.0-or-later`, and likewise for `GPL-3.0`,
  `AGPL-3.0`, `LGPL-2.1` and `LGPL-3.0`. **Which one is the user's
  decision, not yours**, because the difference is substantive: it
  decides whether future FSF versions apply. Ask it once, together
  with the license selection, with the consequence of each stated in
  one line. Do not default to either.
...
- **`Proprietary.txt` has no SPDX identifier.** Use
  ...
  OSI-approved, and Gate 4 in Section 0.2 covers it.
...
`readme.txt` Section 5 holds a numbered reference block, `[01]` to
`[50]`, one per license. Once the license is selected, locate its block
and use it. Do not invent notice wording.
...
| **ÖRNEK 1**, the copyright line | Depends entirely on which of the two ÖRNEK 1 headings the block carries | `Lisans dosyasındaki telif satırı (doldurulmuş)` → written **into** the LICENSE draft, in place of its placeholders. `Telif satırı (LICENSE'a YAZILMAZ; ÖRNEK 2/3'te kullanılır)` → **nothing goes into the LICENSE file**; the line is used only in ÖRNEK 2 and ÖRNEK 3. Read the heading before acting (rule 0.1.3d). |
| **ÖRNEK 2**, the source file header or notice | Every project-owned source file | Used **verbatim** as the pattern, with the real year / holder / software name substituted, adapted only to the file's comment syntax (`//`, `#`, `/* */`, `<!-- -->`, `;`). The legal wording is never paraphrased, shortened, reordered, or translated. |
| **ÖRNEK 3**, the README license note | The project README's license section | Used as the **content model**, then written in the file-output language (see LANGUAGE, English by default). The `readme.txt` samples are written in Turkish; carry over their substance (which license, the one obligation worth naming, the link to the LICENSE file), not their Turkish wording. SPDX identifiers, license names and URLs inside them are reproduced unchanged. |
...
**Behaviour when a gate blocks a license the user asked for: STOP, do
not apply it.** State which gate it hit and why, in one or two
...
`CC-BY-4.0` or bundled fonts under `OFL-1.1`, alongside a real
...
These are not judgment calls, and each is verified per Section 0's
external-verification rule before being asserted:
...
### Gate 6: Ecosystem/context mismatch (soft, so flag it and do not block)
...
Perl calls for `Artistic-2.0`, the EU public sector for `EUPL-1.2`,
the .NET and Microsoft ecosystem for `MS-PL`, an academic institution
for `ECL-2.0` or `NCSA`, the Chinese ecosystem for `MulanPSL-2.0`.
That reason is stated on the same line. Otherwise prefer the mainstream equivalent.
...
`shipped` and `build-only` evidence in 0.3.2 decides, never a guess),
...
advice, which is the same limit that governs everything else in this
document.

**If the user removes this deliverable from the plan**, that is
honoured, but it is written down rather than quietly dropped. The plan
and the Section 6 summary each record one line, "third-party notices: not
...
- If the derived set is empty, that is a legitimate outcome: state it
  ...
  third-party asset") and skip 0.3.3 to 0.3.8. An empty set is asserted
...
1. Compare the candidate texts **byte for byte, after excluding each
   file's copyright lines**. Actually perform the comparison. This
...
3. If they do **not** match, whether from a modified clause, an added
   exception or a different license version, they are **separate
   blocks** inside the same file, each with its own Part A entry and its own Part B, and
...
1. **The component's own license file as installed in this project at
   the resolved version.** This is the path in report Section 10, field 2
...
2. **The component's official repository or registry page at that
   exact version tag**, and only when nothing is installed on disk.
...
the copyright or holder line, report it, and still reproduce the text
...
- **Artifact copy:** wherever this project's distributed artifact can
  carry it, decided from the project's actual build layout, meaning
  the manifest scripts, the bundler and packager configuration and the
  platform project files, never from a habit about "projects like
  this":
  | Project shape | Where the copy goes |
  |---|---|
  | Web / hybrid app whose build copies a web root | that web root (`www/`, `public/`, `dist/`, `src/assets/`), and the copy step then carries it into the packaged app |
  ...
  | Published npm / PyPI / crates package | inside the packaged tree, plus the manifest entry that includes it (`files`, `MANIFEST.in`, `include`), because a file not listed there is not published |
...
1. **Coverage.** Entries in the `LICENSE.txt` register == count of
   ...
   numbers. If they are not equal, it is not complete.
...
Reproduce the text as normal, and additionally name what remains and
which artifact would satisfy it, one line each, in the plan and in the
Section 6 summary. Naming what a license's own text requires
...
## 1. CONTEXT / DOMAIN
The report is project-specific. It may come back clean, with 0
findings, or it may contain dependency and asset license conflicts.
Do not assume in advance, and act according to the report's actual
content.
...
point. Do not assume, and do not proceed to PHASE 1/e until an
...
4. Apply the approved license to the project: correct content in
   ...
   README section and so on, according to the project's language and
...
- [ ] The license status has been clarified for EVERY licensable
      ...
      rule below, and that exclusion satisfies this criterion rather
      than violating it). Scope: files are divided into the following
      classes: project source files, third-party and vendored files,
      generated and build files, binary files, configuration and
      metadata files, and assets. The project SPDX/
...
- [ ] The license field in the manifest file (csproj/
      ...
      without a manifest license field, such as Go, this step is
      N/A and is skipped with the reason stated.
...
- [ ] **No third-party license text was synthesized.** Not from
      `Licenses/`, not from memory, and not by filling a template with
      an upstream author's name (Section 0.3.5). Components whose text
...
b. If the report is partially insufficient (see Section 0
   exception, for example when file contents are not in the report
   and only the status is), examine the relevant project files and
...
d. **Analysis order (the license recommendation is formed in
   ...
   1. **Dependency license compatibility first filter.** If the
      ...
      then reflect it in the recommendation list. If a constraint
      exists, state in one sentence why a license was excluded from
      the list, and don't silently drop it.
   2. **Infer from project context:** project **medium** (the
      Section 1 field: software, font, dataset, content or LaTeX.
      This is what Gate 1 keys off, so establish it before anything
      else), project type (library against application, where a
      library prefers avoiding copyleft and an application can be
      more flexible), target audience (is commercial use expected,
      taken from the "Target use case" field in Section 1),
   ...
   4. **Present the recommendation + list together:** First a
      ...
      reasons"), then briefly list 2 to 6 genuinely applicable
      ...
      otherwise be. By default 2 to 4 candidates suffice, and 5 or 6
      only when meaningful trade-offs exist. If fewer than 2 compatible
   ...
   6. The decision stays with the user. At this step you
      recommend, you do not select, and the final selection is
...
e. **Plan content: produce a COMPLETE plan consisting of
   ...
   2. Per the analysis order in step (d), derive the license
      candidates (2 to 6, default 2 to 4) and their trade-offs, present
      the recommendation.
   ...
   5. Derive the implementation steps based on the approved
      ...
      - **LICENSE file:** the source draft
        ...
        placeholder-to-value substitutions to be made, and the
      ...
      - **SPDX identifier to be used in metadata:** the normalized
        value per Section 0.1.5, including the `-only` and
      ...
      - **Copyright line injection:** a line in the format
        ...
        mark, some append `All rights reserved.`, so follow the
        block and do not normalize).
      ...
      - **Manifest/package file update:** determine the correct
        ...
        - `pyproject.toml`: `[project] license` or
          `license-expression`
        - `Cargo.toml`: `license`
        - `go.mod`: Go has no manifest field, the root `LICENSE`
          alone suffices, so skip this step and state why it was
          skipped.
      ...
      - **Third-party notices folder (Section 0.3):** the
        redistribution set as counts, meaning how many components
        are `shipped`, how many are `UNCERTAIN` and therefore
        included, and how many are excluded as `build-only` (named),
        plus the distinct licenses across that set, each with the number of
        ...
        redistributed.
      For each sub-item, give **where, what and how**: the file path,
      the content to be added, and the format.
...
g. Present this complete plan to the user and STOP. Do not
   ...
   **Combined-message handling:** if a single user message contains
   ...
   begin, and the license name is requested separately before
   proceeding.
...
Steps involving decision branching or that depend on each other, such
as manifest detection followed by the manifest update, are not
merged. For
each step:
a. State which step you are on and what you will examine/attempt.
b. ACTUALLY examine, check or produce that step rather than reason
   ...
   it if absent, add it in an appropriate place if present). These
   are concrete production steps, not skippable through reasoning. The third-party notices folder is built the same
...
- **Minor plan correction** (a step requires a small fix/
  ...
  approach): briefly state it, apply the correction and continue.
...
- **Critical decision points** (a genuine trade-off choice, an
  ...
  unexpected finding with no clear resolution, and ESPECIALLY which
  license to select and how to handle VIOLATION or CONFLICT if
  present in the report): STOP and ask the user directly, as in
  "License
...
**Implementation detail level (secondary decisions such as header
format, which files to exclude, or the manifest field name):**
...
- **If there is a single correct answer** (e.g. if the manifest
  ...
  and undisputed), don't build a criteria table. Apply it directly
  and state the justification in one sentence.
- **If multiple valid approaches exist** (e.g. whether to add the
  header to the top of every file, or only to "original" files, and
  both are defensible), break it down as "If X: approach A. If Y:
...
## COMMUNICATION
This section governs the prose you write and say. It never reaches
license text. Every reproduced license, every copyright notice, every
`readme.txt` sample and every fixed status string is copied byte for
byte as its author wrote it, and none of the rules below apply inside
one. Where this section and Sections 0.1.3, 0.3.4 or 0.3.5 could be
read against each other, those sections win without argument.

### Voice
- Write like someone who has read the report and the drafts, not like a
  compliance form filling itself out.
- Every sentence carries a fact, its evidence, or the consequence of
  the two. No reassurance, no padding, no "licensing was done".
- Recommend plainly and say why. State a refusal in one sentence and
  hold it, without lecturing.
- Make no legal claim. Naming what a license's own text says is
  reading, and it stops there.

### Punctuation and flow
- No em dash and no en dash in anything you author: the plan, the step
  reports, the README license section, the register's own prose, the
  Section 6 summary. Use a comma, a period, a colon or parentheses
  instead. If a sentence only holds together with a dash, it was two
  sentences.
- Reproduced content is exempt and stays exactly as found. That covers
  license texts, upstream copyright lines, `NOTICE` files, the
  `readme.txt` ÖRNEK blocks, placeholder names, SPDX identifiers, file
  paths and the fixed status strings this document quotes.
...
- One idea per sentence, and no clause nested inside a clause inside a
  clause. A finding, the evidence behind it and what follows from it
...
- Avoid the patterns that make text sound machine written: "not X, but
  ...
  labels, phrases like "worth noting".

### Paragraphs
- One paragraph does one job. The recommendation in one, the reasoning
  in the next, the trade-offs after that.
- Leave a blank line between paragraphs, between numbered plan steps,
  and around every table.
- Build the reasoning in order, then land the recommendation. Do not
  open with the verdict and explain it backwards.

### Examples and references
- Every claim about a license points at where it came from: the draft
  file, the `readme.txt` block number, the report section, the
  external source. Name the source first, then say what it shows.
- Calibrate the depth. One line naming the clause and its consequence
  is enough for a candidate comparison. A paragraph of license theory
  buries the choice the user has to make, and a bare identifier proves
  nothing.
- When you point at a file, a dependency or a report row, name it
  before you say what is wrong with it. Do not assume the user is
  looking at the same line you are.
- A verification report is numbers, not adjectives. "21 lines both
  sides, 1 line differs" is a result. "Verified successfully" is not.
- Do not introduce a term the licenses and the report have not already
  used, and never invent notice wording to fill a gap.
...
- Only open project files in the following cases: (a) the relevant
  ...
  (report Section 1.2 absent, empty, or ambiguous), where the
  manifest `author` or `authors` field, existing file headers,
  `AUTHORS` and `CONTRIBUTORS`, and VCS history are all in scope for
  this trigger, or (f) the project type / ecosystem needed for the
  applicability gates (Section 0.2) and the manifest field name is
  not derivable from the report, where manifest files, the root
...
- Never assume. Verify everything that is verifiable, meaning
  dependency license texts, SPDX identifiers and license
  compatibility rules. If multiple sources exist, cross-verify per
  the source priority hierarchy in Section 0, and report conflicts
  without hiding them.
...
- The dependency compatibility step is mandatory only if the
  ...
  silently omitted.
- Do not make the license SELECTION yourself. Recommend, and let the
  decision rest with the user. This constraint follows the same
  principle as "critical decision points" in Section 4, and the
...
- Never manufacture a third-party copyright notice. A holder, a
  ...
  Unresolved (Section 0.3.5), under the same "never assume" rule that
...
- **Placeholders filled:** each one and the value it received
  (`[YEAR]` becomes 2025, `[COPYRIGHT HOLDER]` becomes the holder's
  name), or "none — verbatim copy" when the draft had no
  placeholders.
...
The summary is not a long report, it's a "what was done" list. No
repeating rationale, no repeating the plan.
